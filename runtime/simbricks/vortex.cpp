...
#define ACCESS_REG8(r) (*(volatile uint8_t *) ((uint8_t*) regs + (r)))
...
public:
    ...
    int mem_alloc(uint64_t size, int flags, uint64_t* dev_addr) {
        uint64_t addr;
        ACCESS_REG(SB_INFRA_MEM_SIZE) = size;
        ACCESS_REG(SB_INFRA_MEM_CMD) = SB_INFRA_MEM_CMD_ALLOC;
        addr = ACCESS_REG(SB_INFRA_MEM_ADDR);
        while(ACCESS_REG(SB_INFRA_MEM_CMD) == SB_INFRA_MEM_CMD_ALLOC);
        if(ACCESS_REG(SB_INFRA_MEM_RETURNC)) {
            return ACCESS_REG(SB_INFRA_MEM_RETURNC);
        }
    ...
    int mem_reserve(uint64_t dev_addr, uint64_t size, int flags) {
        ACCESS_REG(SB_INFRA_MEM_ADDR) = dev_addr;
        ACCESS_REG(SB_INFRA_MEM_SIZE) = size;
        ACCESS_REG(SB_INFRA_MEM_FLAGS) = flags;
        ACCESS_REG(SB_INFRA_MEM_CMD) = SB_INFRA_MEM_CMD_RESERVE;
        while(ACCESS_REG(SB_INFRA_MEM_CMD) == SB_INFRA_MEM_CMD_RESERVE);
        if(ACCESS_REG(SB_INFRA_MEM_RETURNC)) {
            return ACCESS_REG(SB_INFRA_MEM_RETURNC);
        }
    ...
    int mem_free(uint64_t dev_addr) {
        ACCESS_REG(SB_INFRA_MEM_ADDR) = dev_addr;
        ACCESS_REG(SB_INFRA_MEM_CMD) = SB_INFRA_MEM_CMD_RELEASE;
        while(ACCESS_REG(SB_INFRA_MEM_CMD) == SB_INFRA_MEM_CMD_RELEASE);
        return ACCESS_REG(SB_INFRA_MEM_RETURNC);
    ...
    int mem_access(uint64_t dev_addr, uint64_t size, int flags) {
        ...
        // TODO: Simbricks Mem Set Access Rights (acl)
        if (flags | VX_MEM_WRITE) {
            flags |= VX_MEM_READ; // ensure caches can handle fill requests
        }
        //  ram_.set_acl(dev_addr, size, flags);
        ACCESS_REG(SB_INFRA_ACL_SADDR) = dev_addr;
        ACCESS_REG(SB_INFRA_ACL_SIZE) = size;
        ACCESS_REG(SB_INFRA_ACL_FLAGS) = flags;
        ACCESS_REG(SB_INFRA_ACL_SET) = true;
    ...
    int mem_info(uint64_t* mem_free, uint64_t* mem_used) const {
        if (mem_free)
            *mem_free = ACCESS_REG(SB_INFRA_MEM_FREE); //global_mem_.free();
        if (mem_used)
            *mem_used = ACCESS_REG(SB_INFRA_MEM_USED); //global_mem_.allocated();
        return 0;
    }
